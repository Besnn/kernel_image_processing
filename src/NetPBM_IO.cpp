...
//        std::vector<u8> * pixel_buffer = new std::vector<u8>(width * height);
        ...
        auto r_channel = std::make_unique<Channel<u16>>(width * height);
        auto g_channel = std::make_unique<Channel<u16>>(width * height);
        auto b_channel = std::make_unique<Channel<u16>>(width * height);

        u32 N = pixel_buffer.size() / 3;
        const u8 * p = pixel_buffer.data();
        for (u32 i = 0; i < N; ++i) {
            r_channel->at(i) = static_cast<u16>(pixel_buffer[i * 3 + 0]);
